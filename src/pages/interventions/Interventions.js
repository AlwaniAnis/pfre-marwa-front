...
import { interventionsMethods } from "../../contexts/TndevContext";
import {
  Button,
  Collapse,
  Container,
...
export default function Interventions() {
  ...
  const { interventions, setinterventions, total, settotal } = states;
  ...
  const { isSuccess, isLoading, refetch, error, data, isFetching } = useQuery(
    ["interventions-all"],
    () => apiInterventionsAll(order, page, rowsPerPage),
    {
      onSuccess: (data) => {
        setinterventions(data.data);
        settotal(data.total);
        //console.log(Interventions[0].user.role);
      },
  ...
  const handleClickOpenDeleteDialog = (id) => {
    let record = interventions?.find((i) => i.id === id);
    //console.log(record);
    ...
    <>
                  ...
                  .map((i) => {
                    return (
                      ...
                      >
                        ...
                        <TableCell align="right">
                          {format(new Date(i.dateIntervention), "dd-MM-yyyy")}
                        </TableCell>
          ...
          >
            <div style={{ marginTop: "1rem" }}>
              ...
              <TextField
                ...
                autoComplete="dateIntervention"
                label="date Intervention"
                helperText=""
                variant="standard"
                sx={{ pt: 2.1 }}
                value={model.dateIntervention}
                onChange={handleOnChange}
              />
        ...
        <DialogContent>
          ...
          >
            <Typography sx={{ mt: 4 }} variant="h6">
              ...
              Confirmez-vous la suppression de cet Intervention ?
            </Typography>
          </Box>
        </DialogContent>
        <DialogActions>
      ...
      >
        <DialogTitle sx={{ color: "white", backgroundColor: green[500] }}>
          Demande de cloture Intervention
        </DialogTitle>
        <DialogContent>
          <Box
            noValidate
          ...
          >
            <Typography sx={{ mt: 4 }} variant="h6">
              {" "}
              Merci de cloturer cet Intervention .
            </Typography>
          </Box>
        </DialogContent>
        <DialogActions>
